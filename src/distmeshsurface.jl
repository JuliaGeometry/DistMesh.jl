function distmeshsurface(fd,fh,h0,bbox::HyperRectangle,varargin)
    ...
    t2n = (t2n.-one(eltype(t2n)))' # TODO there might be assumptions in trisurfup that might bit us later here
    ...
    while true
        ...
        if max(sqrt(sum((p-p0).^2,2))/h0)<dptol
            break
        end
    end
end
