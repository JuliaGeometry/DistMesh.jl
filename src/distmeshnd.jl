...
function distmesh(fdist::Function,fh::Function,h::Number, setup::DistMeshSetup{T,ReTri}=DistMeshSetup(), ::Type{VertType}=GeometryBasics.Point{3,Float64}; origin=VertType(-1,-1,-1),
    ...
    @inbounds while true
        ...
        maxdp = Threads.Atomic{VTE}(typemin(VTE)) # atomics
        maxmove = Threads.Atomic{VTE}(typemin(VTE))
        Threads.@threads for i in eachindex(p)
            ...
            if d < -geps
                ...
                Threads.atomic_max!(maxdp, mdp)
            end

            if d <= setup.iso
                mv = sqrt(sum((p[i]-p0).^2))
                Threads.atomic_max!(maxmove,mv) # determine movements
                continue
            end
